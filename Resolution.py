# Implementation of resolution (EXP 7b)
...
def getInput():
    inputQueries=[]
    inputSentences=[]
    noOfQueries = int(input("Give the no of Queries:"))
    print("Now Give the Queries: ")
    for i in range(noOfQueries):
        x=input()
        inputQueries.append(x)
    noOfSentences = int(input("Give the no of Sentences:"))
    print("Now Give the Sentences: ")
    for j in range(noOfSentences):
        y=input()
        inputSentences.append(y)
...
if __name__ == '__main__':
    ...
    print("Thus the result of the resolution is ",results_)
